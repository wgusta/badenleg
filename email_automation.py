...
EMAIL_SEQUENCE = {
    "day_0_welcome": {
        "delay_days": 0,
        "subject": "Willkommen bei BadenLEG! Ihre Nachbarn warten",
...
def process_email_queue(app=None):
    ...
    for item in pending:
        ...
        config = EMAIL_SEQUENCE.get(template_key)
        if not config:
        ...
        try:
            if app:
                ...
                html_body = f"<p>BadenLEG: {config['subject']}</p>"
