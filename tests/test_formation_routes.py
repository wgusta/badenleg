...
class TestFormationRoutes:
    ...
    def test_create_community(self, mock_create, full_client):
        mock_create.return_value = {
            "community_id": "c1", "name": "Test LEG",
            "status": "interested", "member_count": 1
        }
        resp = full_client.post('/api/formation/create', json={
            "name": "Test LEG", "building_id": "b1",
    ...
    def test_invite_member(self, mock_invite, full_client):
        resp = full_client.post('/api/formation/invite', json={
            "community_id": "c1", "building_id": "b2", "invited_by": "b1"
        })
        assert resp.status_code == 200
    ...
    def test_confirm_membership(self, mock_confirm, full_client):
        resp = full_client.post('/api/formation/confirm', json={
            "community_id": "c1", "building_id": "b2"
        })
        assert resp.status_code == 200
    ...
    def test_start_formation_min_members(self, mock_start, full_client):
        resp = full_client.post('/api/formation/start', json={
            "community_id": "c1"
        })
        assert resp.status_code == 400
    ...
    def test_generate_docs(self, mock_docs, full_client):
        ...
        resp = full_client.post('/api/formation/generate-docs', json={
            "community_id": "c1"
        })
        assert resp.status_code == 200
        data = resp.get_json()
    ...
    def test_submit_dso(self, mock_dso, full_client):
        resp = full_client.post('/api/formation/submit-dso', json={
            "community_id": "c1"
        })
        assert resp.status_code == 200
    ...
    def test_get_status(self, mock_status, full_client):
        mock_status.return_value = {
            "community_id": "c1", "name": "Test LEG",
            "status": "interested", "member_count": {"total": 3, "confirmed": 3}
        }
