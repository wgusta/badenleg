...
class TestBillingCronWiring:
    ...
    def test_billing_cron_calls_engine(self, mock_communities, mock_members, mock_readings, mock_save, mock_engine, full_client):
        ...
        mock_engine.return_value = {
            'total_production_kwh': 1.1, 'total_allocated_kwh': 0.8,
            'total_surplus_kwh': 0.3, 'total_network_discount_chf': 0.05,
            'participants': []
        }
        resp = full_client.post('/api/cron/process-billing',
    ...
    @patch('database.get_active_communities')
    def test_billing_cron_fetches_member_buildings(self, mock_communities, mock_members, mock_readings, mock_save, mock_engine, full_client):
        mock_communities.return_value = [{'community_id': 'c1', 'distribution_model': 'proportional'}]
        mock_members.return_value = ['b1', 'b2']
        mock_readings.return_value = MOCK_READINGS
        mock_engine.return_value = {
            'total_production_kwh': 1.0, 'total_allocated_kwh': 0.5,
            'total_surplus_kwh': 0.5, 'total_network_discount_chf': 0.02,
            'participants': []
        }
        resp = full_client.post('/api/cron/process-billing',
            headers={'X-Cron-Secret': 'test-cron-secret'})
    ...
    @patch('database.get_active_communities')
    def test_billing_cron_builds_dataframes(self, mock_communities, mock_members, mock_readings, mock_save, mock_engine, full_client):
        mock_communities.return_value = [{'community_id': 'c1', 'distribution_model': 'proportional'}]
        mock_readings.return_value = MOCK_READINGS
        mock_engine.return_value = {
            'total_production_kwh': 1.0, 'total_allocated_kwh': 1.0,
            'total_surplus_kwh': 0, 'total_network_discount_chf': 0.04,
            'participants': []
        }
        resp = full_client.post('/api/cron/process-billing',
            headers={'X-Cron-Secret': 'test-cron-secret'})
    ...
    @patch('database.get_community_member_building_ids', return_value=['b1'])
    @patch('database.get_active_communities')
    def test_billing_cron_saves_period(self, mock_communities, mock_members, mock_readings, mock_save, mock_engine, full_client):
        mock_communities.return_value = [{'community_id': 'c1', 'distribution_model': 'proportional'}]
        mock_readings.return_value = MOCK_READINGS
        mock_engine.return_value = {
            'total_production_kwh': 0.5, 'total_allocated_kwh': 0.5,
            'total_surplus_kwh': 0, 'total_network_discount_chf': 0.02,
            'participants': []
        }
        full_client.post('/api/cron/process-billing',
            headers={'X-Cron-Secret': 'test-cron-secret'})
    ...
    @patch('database.get_community_member_building_ids', return_value=['b1'])
    @patch('database.get_active_communities')
    def test_billing_cron_returns_count(self, mock_communities, mock_members, mock_readings, mock_save, mock_engine, full_client):
        mock_communities.return_value = [{'community_id': 'c1', 'distribution_model': 'proportional'}]
        mock_readings.return_value = MOCK_READINGS
        mock_engine.return_value = {
            'total_production_kwh': 0.5, 'total_allocated_kwh': 0.5,
