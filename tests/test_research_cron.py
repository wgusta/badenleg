...
class TestSearchBrave:
    ...
    def test_returns_parsed_results(self, mock_get):
        ...
        mock_get.return_value = MagicMock(
            status_code=200,
            json=lambda: MOCK_BRAVE_RESPONSE
        )
...
class TestSummarizeWithLLM:
    ...
    def test_returns_summary_string(self, mock_post):
        ...
        mock_post.return_value = MagicMock(
            status_code=200,
            json=lambda: MOCK_GROQ_RESPONSE
        )
        summary = summarize_with_llm("BFE grants", [
            {"title": "BFE Pilot 2026", "url": "https://bfe.admin.ch", "description": "New call"}
        ])
...
class TestAppendToResearchMd:
    ...
    def test_updates_freshness_index(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.md', delete=False) as f:
            ...
            assert "Not started" not in content or content.count("Not started") < RESEARCH_MD_HEADER.count("Not started")
            os.unlink(f.name)
