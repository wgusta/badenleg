...
class TestDashboardTokenGating:
    ...
    def test_dashboard_no_token_returns_403(self):
        ...
        assert "request.args.get('bid')" not in content, \
            "Dashboard still uses ?bid= query param (IDOR)"

    def test_dashboard_requires_token_lookup(self):
        with open(os.path.join(PROJECT_ROOT, "app.py")) as f:
            content = f.read()
        assert "get_token" in content or "_require_dashboard_token" in content, \
            "Dashboard must validate token from DB"
    ...
    def test_dashboard_token_type_exists(self):
        ...
        assert "'dashboard'" in content, \
            "No dashboard token type found in app.py"
    ...
    def test_meter_upload_checks_token(self):
        ...
        for line in lines:
            if 'def api_meter_data_upload' in line:
                ...
                break
            elif in_meter_fn and ('get_token' in line or '_require_dashboard_token' in line or 'dashboard' in line.lower()):
                has_token_check = True
        assert has_token_check, \
            "Meter upload does not verify dashboard token"
...
class TestUnsubscribeNoDelete:
    ...
    def test_unsubscribe_post_no_delete(self):
        ...
        assert not calls_delete, \
            "POST /unsubscribe still calls delete_building directly"
