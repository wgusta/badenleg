...
class TestCheckReadyCommunities:
    ...
    def test_check_ready_returns_eligible(self, mock_db):
        mock_db.return_value = [
            {'community_id': 'c1', 'name': 'LEG Dietikon', 'member_emails': ['a@x.ch', 'b@x.ch', 'c@x.ch'], 'confirmed_count': 3},
        ]
        from email_automation import check_formation_ready_communities
...
class TestEmailCronNudges:
    ...
    def test_email_cron_triggers_nudges(self, mock_pending, mock_expire, mock_track, mock_check, mock_nudge, full_client):
        mock_check.return_value = [
            {'community_id': 'c1', 'name': 'Test LEG', 'member_emails': ['a@x.ch', 'b@x.ch', 'c@x.ch']},
        ]
        resp = full_client.post('/api/cron/process-emails',
