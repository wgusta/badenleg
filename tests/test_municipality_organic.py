...
def test_municipality_profile_states_its_assumed_consumption_from_the_calculation():
    ...
    text = (ROOT / "templates" / "gemeinde" / "profil.html").read_text(encoding="utf-8")
    assert "value_gap.assumed_consumption_kwh" in text
    assumption_line = next(line for line in text.splitlines() if "Annahme:" in line)
...
def test_leg_kalkulator_names_its_fee_assumption():
    ...
    text = (ROOT / "templates" / "leg_kalkulator.html").read_text(encoding="utf-8")
