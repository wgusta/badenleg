...
class TestLocalizedPages:
    ...
    def test_landing_french_tenant(self, mock_tenant, full_app):
        mock_tenant.return_value = {
            'territory': 'lausanne', 'city_name': 'Lausanne',
            'kanton': 'Vaud', 'kanton_code': 'VD', 'language': 'fr',
            'platform_name': 'OpenLEG', 'brand_prefix': 'OpenLEG',
            'utility_name': 'Romande Energie', 'primary_color': '#0d9488',
            'secondary_color': '#f59e0b', 'contact_email': 'hallo@openleg.ch',
            'dso_contact': '', 'legal_entity': '',
            'map_center_lat': 46.52, 'map_center_lon': 6.63, 'map_zoom': 12,
            'map_bounds_sw': [46.4, 6.5], 'map_bounds_ne': [46.6, 6.8],
            'solar_kwh_per_kwp': 1000, 'active': True,
        }
    ...
    def test_landing_italian_tenant(self, mock_tenant, full_app):
        mock_tenant.return_value = {
            'territory': 'lugano', 'city_name': 'Lugano',
            'kanton': 'Ticino', 'kanton_code': 'TI', 'language': 'it',
            'platform_name': 'OpenLEG', 'brand_prefix': 'OpenLEG',
            'utility_name': 'AIL', 'primary_color': '#0d9488',
            'secondary_color': '#f59e0b', 'contact_email': 'hallo@openleg.ch',
            'dso_contact': '', 'legal_entity': '',
            'map_center_lat': 46.0, 'map_center_lon': 8.95, 'map_zoom': 12,
            'map_bounds_sw': [45.9, 8.8], 'map_bounds_ne': [46.1, 9.1],
            'solar_kwh_per_kwp': 1100, 'active': True,
        }
        with full_app.test_client() as c:
            resp = c.get('/')
            html = resp.data.decode()
