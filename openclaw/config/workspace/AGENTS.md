...
All tools are available via the CLI. Use exec to run:
```
node /opt/mcp-badenleg-server/cli.mjs <tool_name> [--param value ...]
```

Examples:
```
node /opt/mcp-badenleg-server/cli.mjs get_stats
node /opt/mcp-badenleg-server/cli.mjs list_communities --status active
node /opt/mcp-badenleg-server/cli.mjs search_registrations --plz 5400
node /opt/mcp-badenleg-server/cli.mjs upsert_tenant --territory_slug baden --config '{"city_name":"Baden","kanton":"AG"}'
node /opt/mcp-badenleg-server/cli.mjs help
```

Output is always JSON. Parameters use `--key value` format. JSON parameters (like config) are passed as JSON strings.
...
- `send_telegram` - Send message to CEO via Telegram (categories: progress, blocked, approval_needed, daily_report, alert)
- `track_strategy_item` - Upsert strategy tracker entry (week 1-12, item slug, status, notes)
- `get_strategy_status` - Read strategy tracker with per-week summary counts
...
| `strategy-standup` | 08:00 Zurich, every day | Check strategy status, pipeline metrics, community health. Send Telegram daily_report. Flag needs_ceo items. |
| `vnb-outreach-cycle` | 09:00 Zurich, Tue/Thu | Research top uncontacted VNBs, draft outreach, save as needs_ceo, send Telegram approval_needed. |
| `pipeline-review` | 10:00 Zurich, Fridays | Funnel metrics, stale leads >14d, stuck formations. Weekly Telegram report. |
